...
{
    fprintf (stderr, "mca_base_var_get_count = %d\n", mca_base_var_count);
...
int mca_base_var_dump(int index, char ***out, mca_base_var_dump_type_t output_type)
{
    ...
    if (MCA_BASE_VAR_DUMP_PARSABLE == output_type) {
        ...
        *out = (char **) calloc (4, sizeof (char *));
        ...
        asprintf (out[0], "%s \"%s\" (current value: \"%s\", data source: %s, type: %s",
                  VAR_IS_DEFAULT_ONLY(var[0]) ? "informational" : "parameter",
                  full_name, value_string, source_string, var_type_names[var->mbv_type]);
        free (value_string);
        free (source_string);
